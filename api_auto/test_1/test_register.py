...
class TestRegister(unittest.TestCase, APICase):
    ...
    def setUp(self):
        # 发送验证码
        fk = Faker(locale=['zh-CN'])     #切换中国，才能造中国号码
        ...
        code = self.db.query_one(sql)[0]

        # 校验验证码, 设置的属性要和 excel 当中 #...# 当中的字符串保持一致。
    ...
    def test_admin_login(self, item):
        ...
        # TODO: 数据替换
        # data = data.replace('#smsflag#', self.smsflag)
